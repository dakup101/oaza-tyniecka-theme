<?php $banner = get_field('hero_banner'); ?>

<div class="w-full h-screen relative overflow-hidden hero-shadow">
    <!-- Parallax -->
    <img src="<?php echo $banner['img'] ?>" alt="Oaza Tyniecka - mieszkania w nowych domach"
        class="parallax h-full w-auto" data-scale="1.3" data-transition="cubic-bezier(0,0,0,1)" data-orientation="up"
        data-delay="0.5">
    <!-- Overlay -->
    <div class="absolute w-full h-full top-0 left-0 overlay-bg"></div>
    <!-- Content -->
    <div class="absolute w-full h-full top-0 left-0 flex flex-col items-center justify-center text-white">
        <h1 class="text-4xl sm:text-6xl lg:text-8xl text-center opacity-90">
            <?php echo $banner['title'] ?>
        </h1>
        <p class="text-center text-xl sm:text-2xl lg:text-4xl my-14 text-yellow">
            <?php echo $banner['subtitle'] ?>
        </p>
        <a href="<?php echo $banner['btn_link'] ?>" class="btn-white text-lg change-fill-black">
            <?php echo $banner['btn_text'] ?>
            <figure class="w-5 mt-1 ml-5">
                <svg id="Warstwa_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 15">
                    <path class="fill-yellow"
                        d="M21.88,8l-7.53,6.24,.64,.76,9-7.52L15,0l-.65,.76,7.53,6.24H0v1H21.88Z" />
                </svg>
            </figure>
        </a>
    </div>
    <figure data-scroll_icon
        class="absolute bottom-10 left-1/2 -translate-x-1/2 text-white text-xs font-light flex justify-center items-center flex-col transition-all">
        <span><?php echo __("Przewiń w dół", "oaza-tyniecka-theme") ?></span>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 220.84 451.35" class="fill-yellow w-5 mt-5 bounce">
            <path
                d="M110.42,0C49.53,0,0,41.65,0,92.85v177.29c0,47.71,43.01,87.11,98.13,92.26v-8.26c-50.58-5.04-89.92-40.78-89.92-84V92.85C8.21,46.18,54.06,8.21,110.42,8.21s102.2,37.97,102.2,84.64v177.29c0,45.53-43.64,82.76-98.09,84.55V174.12h-8.22V435.63l-23.04-23.04-5.81,5.81,32.96,32.96,32.96-32.96-5.81-5.81-23.04,23.04v-72.73c58.98-1.83,106.31-42.72,106.31-92.76V92.85C220.84,41.65,171.3,0,110.42,0Z" />
            <path
                d="M143.72,89.91c0-18.37-14.94-33.3-33.3-33.3s-33.3,14.94-33.3,33.3,14.94,33.3,33.3,33.3,33.3-14.94,33.3-33.3Zm-58.39,0c0-13.84,11.26-25.09,25.09-25.09s25.09,11.26,25.09,25.09-11.25,25.09-25.09,25.09-25.09-11.25-25.09-25.09Z" />
        </svg>
    </figure>
</div>